...
widths(ws,{"B":14,"C":6,"D":13,"E":11,"F":12,"G":11,"H":13,"I":16,"J":13,"K":12,"L":13,"M":11,"N":12,"O":11,"P":10,"Q":34})
title(ws,"DAILY LOG",
...
HEAD1=r
for cols,lab,fill in [((5,8),"GE1: PAID MEDIA",F_BLUE),((9,13),"GE2: SCHOOL OUTREACH",F_ORANGE),((14,17),"CALCULATED",F_GREY)]:
...
r+=1
HDR=r
r=header(ws,r,["Date","Day","Spend (R)","Clicks","Signups","Sales",
               "Schools contacted","Meetings or presentations secured","Learners reached","Leads captured","Registrations",
...
for i,d in enumerate(DAYS):
    ...
    m=ws.cell(row=rr,column=13,value="=G%d+L%d"%(rr,rr)); m.number_format=NUM; m.font=BODY_B; m.fill=F_GREY
    n=ws.cell(row=rr,column=14,value="=SUM($M$%d:M%d)"%(FIRSTD,rr)); n.number_format=NUM; n.font=BODY_B; n.fill=F_GREY
    o=ws.cell(row=rr,column=15,value="=ROUND(%s/%s*C%d,0)"%(A["qt"],A["days"],rr)); o.number_format=NUM; o.font=MUTED; o.fill=F_GREY
    p=ws.cell(row=rr,column=16,value="=N%d-O%d"%(rr,rr)); p.number_format='+#,##0;-#,##0;0'; p.font=BODY_B; p.fill=F_GREY
    q=ws.cell(row=rr,column=17); q.fill=F_YELLOW; q.font=INPUT
LASTD=FIRSTD+13
r=LASTD+1
ws.cell(row=r,column=2,value="TOTAL").font=BAND; ws.cell(row=r,column=2).fill=F_NAVY
for i in range(2,18): ws.cell(row=r,column=i).fill=F_NAVY
for col,fmt in [(4,CUR),(5,NUM),(6,NUM),(7,NUM),(8,NUM),(9,NUM),(10,NUM),(11,NUM),(12,NUM),(13,NUM)]:
...
TOT=r
c=ws.cell(row=r,column=15,value="=%s"%A["qt"]); c.font=Font(name="Arial",sz=10,b=True,color="FFFFFFFF"); c.number_format=NUM; c.alignment=RGT
c=ws.cell(row=r,column=16,value="=M%d-O%d"%(TOT,TOT)); c.font=Font(name="Arial",sz=10,b=True,color="FFFBBF24"); c.number_format='+#,##0;-#,##0;0'; c.alignment=RGT
r+=2
...
r+=1
ex=["Mon 24 Aug","1","R2 400","310","44","6","6","2","0","0","0","6","6","4","+2","Two schools said yes to a presentation. No learners reached yet."]
...
for t in ["Spend is what the platform reports as spent that day, not what was budgeted.",
          "Sales come from the Gradesmatch tech feed of cleared Peach transactions, split by the UTM the buyer arrived through. A sale with no UTM goes in neither engine column and is noted.",
          "GE2 has five columns because a school visit produces four things before it produces a sale. Log all of them: the work is real even on a day the sales column stays at zero.",
          "Registrations is the only GE2 column that counts as a sale. It stays at zero until a presentation has actually been delivered to learners.",
